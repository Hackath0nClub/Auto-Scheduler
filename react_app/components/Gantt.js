...
const Gantt = () => {
  ...
  const searchRecursiveDependLinks = (id) => {
  ...
  const addDate = (task, days) => {
    const formatFunc = gantt.date.date_to_str("%Y-%m-%d %H:%i");
    return formatFunc(gantt.date.add(task.start_date, days / 60000, "minute"));
  };

  const createUpdateData = () => {
    const currentData = gantt.serialize();
    const dragDate = afterStartDate - beforStartDate;
    const addDateData = currentData.data.map((task) => {
